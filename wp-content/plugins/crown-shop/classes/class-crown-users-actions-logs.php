...
if ( ! class_exists( 'Crown_Shop_NSI_Users_Actions_Logs' ) ) {
    class Crown_Shop_NSI_Users_Actions_Logs
    {
        ...
        {
            self::$roles_to_log_activity = array(
                'administrator', 'pricing', 'editor'
            );
        }
        ...
        private static function is_user_role_valid_for_logging(): bool {
            ...
            $user = wp_get_current_user();
            $user_role = $user->roles[0] ?? '';
            return in_array( $user_role, self::$roles_to_log_activity );
        }
        ...
        {
            if ( $original_user_meta->roles[0] === 'administrator' && in_array( $switched_to_meta->roles[0], array( 'editor', 'pricing' ) ) ) {
